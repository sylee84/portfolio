function customerInputCheck() {
    if(jQuery('#memberNm').val() === '') {
        Storm.LayerUtil.alert("이름을 입력해주세요.").done(function() {
            $('#memberNm').focus();
        });
        return false;
    }
    if(jQuery('#loginId').val() === '') {
        Storm.LayerUtil.alert("아이디를 입력해주세요.").done(function() {
            $('#loginId').focus();
        });
        return false;
    }
    ...
    // 사업자,일반 구분
    if($('input:radio[name=ra_bizYn]').length > 0) {
        if($('#bizYn_y').is(":checked") == true){
            $('#bizYn').val('Y');
        }else{
            $('#bizYn').val('N');
        }
    ...
    if($('input:radio[name=ra_bizYn]:checked').val() == 'Y'){
        if(!Storm.validation.isEmpty($("#biz_no_01").val()) && !Storm.validation.isEmpty($("#biz_no_02").val()) && !Storm.validation.isEmpty($("#biz_no_03").val())){
            $('#bizNo').val($('#biz_no_01').val()+'-'+$('#biz_no_02').val()+'-'+$('#biz_no_03').val());
        }else{
            $('#bizNo').val('');
        }
        if(!Storm.validation.isEmpty($("#biz_tel01").val()) && !Storm.validation.isEmpty($("#biz_tel02").val()) && !Storm.validation.isEmpty($("#biz_tel03").val())){
            $('#custTel').val($('#biz_tel01').val()+'-'+$('#biz_tel02').val()+'-'+$('#biz_tel03').val());
        }else{
            $('#custTel').val('');
        }
        if(!Storm.validation.isEmpty($("#biz_mobile01").val()) && !Storm.validation.isEmpty($("#biz_mobile02").val()) && !Storm.validation.isEmpty($("#biz_mobile03").val())){
            $('#custMobile').val($('#biz_mobile01').val()+'-'+$('#biz_mobile02').val()+'-'+$('#biz_mobile03').val());
        }else{
            $('#custMobile').val('');
        }
    }
...
function birthInputCheck(){
    ...
    if(year === '') {
        Storm.LayerUtil.alert("년도를 선택해 주세요").done(function() {
            $('#select_id_year').focus();
        });
        return false;
    }
    if(month === '') {
        Storm.LayerUtil.alert("월을 선택해 주세요").done(function() {
            $('#select_id_month').focus();
        });
        return false;
    }
    if(date === '') {
        Storm.LayerUtil.alert("일을 선택해 주세요").done(function() {
            $('#select_id_date').focus();
        });
        return false;
    }
...
function deliveryInputCheck() {
    if(jQuery('#gbNm').val() === '') {
        Storm.LayerUtil.alert("배송지명을 입력해주세요").done(function() {
            $('#gbNm').focus();
        });
        return false;
    }
...
function checkPost(){
    if($('#shipping_internal').is(":checked") == true){
        ...
        if(Storm.validation.isEmpty($("#newPostNo").val())) {
            Storm.LayerUtil.alert("우편번호를 입력해주세요.").done(function() {
                $('#newPostNo').focus();
            });
            return false;
        }
        if(Storm.validation.isEmpty($("#strtnbAddr").val()) && Storm.validation.isEmpty($("#roadAddr").val())) {
            Storm.LayerUtil.alert("주소를 입력해주세요.").done(function() {
                $('#strtnbAddr').focus();
            });
            return false;
        }
        /*
        if(Storm.validation.isEmpty($("#strtnbAddr").val())) {
        ...
        */
        if(Storm.validation.isEmpty($("#dtlAddr").val())) {
            Storm.LayerUtil.alert("상세주소를 입력해주세요.").done(function() {
                $('#dtlAddr').focus();
            });
            return false;
        }
        ...
        if(Storm.validation.isEmpty($("#frgAddrZipCode").val())) {
            Storm.LayerUtil.alert("Zip을 입력해주세요.").done(function() {
                $('#frgAddrZipCode').focus();
            });
            return false;
        }
        if(Storm.validation.isEmpty($("#frgAddrState").val())) {
            Storm.LayerUtil.alert("State를 입력해주세요.").done(function() {
                $('#frgAddrState').focus();
            });
            return false;
        }
        if(Storm.validation.isEmpty($("#frgAddrCity").val())) {
            Storm.LayerUtil.alert("City를 입력해주세요.").done(function() {
                $('#frgAddrCity').focus();
            });
            return false;
        }
        if(Storm.validation.isEmpty($("#frgAddrDtl1").val())) {
            Storm.LayerUtil.alert("address1를 입력해주세요.").done(function() {
                $('#frgAddrDtl1').focus();
            });
            return false;
        }
        if(Storm.validation.isEmpty($("#frgAddrDtl2").val())) {
            Storm.LayerUtil.alert("address2를 입력해주세요.").done(function() {
                $('#frgAddrDtl2').focus();
            });
            return false;
        }
...
function popupAutoResize() {
    ...
    var marginY = 0;
    //Storm.LayerUtil.alert(thisX + "===" + thisY);
    //Storm.LayerUtil.alert("임시 브라우저 확인 : " + navigator.userAgent);
...
function passwordCheck(val){
    ...
    var pass_level =0;
    //Storm.LayerUtil.alert("숫자 : "+len_num+"\n 영문자 : "+len_eng+"\n 특수문자 : "+len_asc);
