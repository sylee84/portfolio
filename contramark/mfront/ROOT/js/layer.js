...
Storm.LayerUtil = {
    ...
    confirm_title : '확인',
    desc : '',
    alert_template : '<div id="div_id_alert" class="alert_body"><div class="pop_wrap">' +
    '<button type="button" class="btn_alert_close"><img src="'+MOBILE_CONTEXT_PATH+'/front/img/common/btn_close_popup02.png" alt="팝업창닫기"></button>' +
    ...
    '</div>' +
    '</div></div>',
    confirm_template : '<div id="div_id_confirm" class="alert_body"><div class="pop_wrap">' +
    '<button type="button" class="btn_alert_close"><img src="'+MOBILE_CONTEXT_PATH+'/front/img/common/btn_close_popup02.png" alt="팝업창닫기"></button>' +
    '<div class="alert_content"><div class="alert_text">{{msg}}</div>' +
    '<div class="alert_btn_area">' +
    ...
    create : function($layer) {
        ...
        $('body').css('overflow-y','hidden').on('touchmove', function(e) {e.preventDefault()});
    ...
    close : function(id) {
        ...
        $body.css('overflow-y','scroll').off('touchmove');
    },
...
//        var left = ( $(window).scrollLeft() + ($(window).width() - $popup.width()) / 2 ),
        var left = 0,
            ...
            dimmed = jQuery('.dimmed').length > 0 ? true : false;
        $popup.fadeIn();
        $popup.css({top: top, left: left, width: '100%'});
        if(dimmed) {
    ...
    zipcode :function(callback) {
        var template ='',
            elementLayer,
            width = 100, //우편번호서비스가 들어갈 element의 width
            height = 100, //우편번호서비스가 들어갈 element의 height
