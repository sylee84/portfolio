...
jQuery(document).ready(function() {
    ...
    $('#a_id_logout').on('click', function(e) {
        ...
        Storm.FormUtil.submit(Constant.FRONT_PATH + '/login/logout.do', {});
    });

    Storm.design();

    //이미지가 없을 경우 no image 처리
    $('img').load().on('error', function(e) {
        if($(this).parents('.exceptionImg').length == 0) {
            var $this = $(this);
            $this.attr('src','/m/front/img/common/no_img.png');
            $this.addClass('no');
            if($this.parents('.main_typeF_con').length > 0) {
                $this.after('<img src="/m/front/img/common/no_img_bg2.gif" alt="">');
            } else {
                $this.after('<img src="/m/front/img/common/no_img_bg.gif" alt="">');
            }
...
Storm.waiting = {
    ...
    start : function() {
        $.blockUI({
            message : '<img src="' + MOBILE_CONTEXT_PATH +'/front/img/ajax-loader-white.gif" alt="Loading..." />'
        });
    },
        ...
        $.ajax({
            ...
            if (xhr.status == 403) {
                Storm.LayerUtil.confirm('로그인 정보가 없거나 유효시간이 지났습니다.<br/>로그인페이지로 이동하시겠습니까?',
                    function() {
                        document.location.href = MOBILE_CONTEXT_PATH+'/front/login/viewLogin.do';
                    });
            } else {
                if (xhr.responseJSON && xhr.responseJSON.message) {
                    console.log(xhr.responseJSON.message);
                    Storm.LayerUtil.alert(xhr.responseJSON.message, callback);
                } else {
                    Storm.LayerUtil.alert("처리중 오류가 발생했습니다. 관리자에게 문의하십시오.", callback);
                }
            }
...
Storm.FileUpload = {
    ...
    fileForm : function(type){
        ...
        html += '<form name="fileUploadForm" id="form_id_fileUploadForm" method="post" enctype="multipart/form-data" class="blind">';
        ...
        $("#file_id_commonUpload")
            .on('change', function() {
                $('#form_id_fileUploadForm').ajaxSubmit({
                    url : '/admin/common/fileUploadResult.do'
                    , dataType : 'json'
        ...
        for(var i = 0, len = $files.length; i < len; i++) {
            ...
            for(var j = 0, l = files.length; j < l; j++) {
                ...
                if(file.size > Constant.file.maxSize) {
                    Storm.LayerUtil.alert('파일 ' + file.name + '의 파일사이즈가 5MB를 초과합니다.');
                    return false;
                }
            }
        }
...
Storm.FileDownload = {
    download : function(filePath, fileName) {
        var url = Constant.FRONT_PATH +'/common/download.do',
        key,
        param = {};
...
Storm.DaumEditor = {
     ...
     */
    init : function() {
        var url = Constant.FRONT_PATH +'/daumeditor/editor.html';
